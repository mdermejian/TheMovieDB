...
extension DataRequest {
		...
		-> Self
	{
		let responseSerializer = DataResponseSerializer<T> { request, response, data, error in
			guard error == nil else { return .failure(OLXError.network(error: error!)) }
			...
			guard case let .success(jsonObject) = result
					...
					return .failure(OLXError.jsonSerialization(error: result.error!))
			}
			
			guard let response = response, let responseObject = T(response: response, representation: jsonObject) else {
				return .failure(OLXError.objectSerialization(reason: "JSON could not be serialized: \(jsonObject)"))
			}
